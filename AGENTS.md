...
本站为 CodeT（aiapi.corrin.cc）运营的 fork，与上游 QuantumNous/new-api 保持 sync（`sync-upstream.yml` 每小时快进 main + 同步 tag；ai 仅在上游新 tag 时合并 main，见「分支约定」）。fork 自有改动：
...
- **分支约定（2026-08-01 重构，2026-08-07 同步机制修正）**：`main` 是**上游纯镜像**——零 fork 提交、零 workflow 文件（仅允许 bot 的 `[skip ci]` 空提交），永远快进；若检测到人工提交说明有人直接推了 main（workflow 自动开 issue 报警，修复：`git push -f origin upstream/main:main`）。`ai` 是**默认分支**与 fork 唯一工作分支：日常开发、全部 workflow、构建、发布全在 ai。**ai 永不合并进 main，main 也绝不承载任何 fork 文件（含 .github/workflows/，旧 A 方案已废弃）**。sync workflow 每小时快进 main + 同步 tag；**仅当上游出现新 tag（或 dispatch 勾选 force_build）时才把 main 合并进 ai**——ai 只在需要构建时吸收上游代码。**main/ai 的推送一律走 `secrets.RELEASE_PAT`（不能用 merge-upstream API 或 GITHUB_TOKEN git push：上游一旦改动 `.github/workflows/` 文件，GitHub App token 无 workflows 权限会被 422 拒绝，2026-08-07 上游新增 `sync-release-to-gitcode.yml` 时实测踩中）；PAT 推送会触发 push 事件，bot 提交一律带 `[skip ci]` 抑制**。合并冲突（常见于 `web/src/i18n/locales/*.json`，上游也频繁改）时 workflow 自动开 issue「main → ai 合并冲突」转人工，本次跳过构建（`ai_ready=false` 闸门），不会用旧 ai 代码误标新版本。locale 冲突建议按 key 三方合并：以 merge-base 算出 fork 独有/改动 key，overlay 到 upstream 版本上。上游新增 workflow 合并进 ai 后默认开启，sync 每次运行会把白名单（`sync-upstream.yml`/`release.yml`）外的 workflow 自动禁用。
- **Release 触发**：version job 推 tag 用 `secrets.RELEASE_PAT`（PAT 推 tag 才能触发 `release.yml` 生成带三平台二进制的 GitHub Release；GITHUB_TOKEN 推 tag 不触发其他 workflow）。**需在仓库 Secrets 配置 `RELEASE_PAT`（Contents+Workflows 写权限；sync job 同步 main/ai 同样依赖它，缺失时 sync 直接报错）**，未配置时回退 GITHUB_TOKEN 并告警（Release 需本机手工重推 tag 触发）。
